...
void static inline stuck() {while(1);}
char buf[BUFSZ];
...
writebig(char *s)
{
  ...
  for(i = 0; i < MAXFILE; i++){
    ...
    if((n = write(fd, buf, BSIZE)) != BSIZE){
      // printf("%s: error: write big file failed, n = %d\n", s, i, n);
      exit(1);
    } else {
      // printf("write big file %d success\n", i);
    }
  }
...
killstatus(char *s)
{
  ...
  for(int i = 0; i < 10; i++){
    ...
    // printf("process %d waiting child process %d...\n", getpid(), pid1);
    wait(&xst);
    // printf("process %d wait child process %d status %d\n", getpid(), pid1, xst);
