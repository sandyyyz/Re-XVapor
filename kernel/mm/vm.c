...
uint64 walkaddr_with_cow(pagetable_t pagetable, uint64 va) {
  pte_t *pte;
  uint64 pa;
  ...
  if (*pte & PTE_COW) {
    ...
    // 重新映射
    // 照理来说应该减少原来物理页的引用, 但是在uvmunmap中设置了do_free = 1
    // 这会调用改动后的kfree
...
void uvmfirst(pagetable_t pagetable, uchar *src, uint sz) {
  ...
  // mappages(pagetable, PGSIZE, PGSIZE, (uint64)testmem,
...
int uvmcopy(pagetable_t old, pagetable_t new, uint64 sz) {
  ...
  for (i = 0; i < sz; i += PGSIZE) {
    ...
    *pte = PA2PTE(pa) | flags;
    ...
    /*
      when cow, just mappages the same page to the new process
      dont alloc new page
    */
...
int copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len) {
  ...
  while (len > 0) {
    ...
    pa0 = walkaddr_with_cow(pagetable, va0);
    if (pa0 == -1)
      return -1;
    n = PGSIZE - (dstva - va0);
    if (n > len)
...
/// @brief allocate PGSIEZE memory for trapframe and map it to thread TRAMPFRAME
/// space
...
struct trapframe *uvm_thread_trapframe(pagetable_t pagetable, int thread_idx) {
  ...
  if (mappages(pagetable, THREAD_TRAPFRAME(thread_idx), PGSIZE, pa,
               PTE_R | PTE_W) < 0) {
    kfree((void *)pa);
    return NULL;
  }
  ...
  Log("uvm_thread_trapframe: thread_idx %d, pa %p, va %p", thread_idx, pa,
      THREAD_TRAPFRAME(thread_idx));
...
void _vmprint(pte_t *pte, int level) {
  if (level == 3) {
    return;
  }
