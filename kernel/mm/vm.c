...
#ifdef __DEBUG_FREEWALK
      Info("thread %d's trapframe in freewalk: %p\n", mythread()->tid, mythread()->trapframe);
      Info("panic pte %p, pa %p\n", pte,PTE2PA(pte));
#endif
      panic("freewalk: leaf");
    }
    //若均无效且无权限(到达已经被移除的叶映射节点)，则不会继续往下递归
  }
...
struct trapframe *uvm_thread_trapframe(pagetable_t pagetable, int thread_idx) {
  ...
  if (mappages(pagetable, THREAD_TRAPFRAME(thread_idx), PGSIZE, pa, PTE_W) < 0) {
      kfree((void *)pa);
      return NULL;
  }
...
pagetable_t kvmmake(void) {
  ...
  // kpgtble可以用来存内核栈的映射，别的时候需要访问内核内存时，使用映射窗口就行
  thread_mapstacks(kpgtbl);
  w_csr_pgdl((uint64)kpgtbl);
  tlbinit();
  // DIR4WIDTH == 0， 那么这不还是三级页表吗？
  // 我知道了，目录项三级，最后还有一级页表。没问题，risc-v是两级目录一级索引，总共三级。
  // 所以loongarch可以支持五级页表
...
uvmfirst(pagetable_t pagetable, uchar *src, uint sz)
{
  ...
  for(uint64 i = 0; i < sz; i += PGSIZE){
    ...
    if(mem == 0)
      panic("uvmfirst: kalloc failed");
    memset(mem, 0, PGSIZE);
    if(mappages(pagetable, i, PGSIZE, (uint64)mem, PTE_P|PTE_W|PTE_PLV|PTE_MAT) != 0){
      kfree(mem);
      panic("uvmfirst: mappages failed");
    }
    memmove(mem, src + i, PGSIZE < sz - i ? PGSIZE : sz - i);
...
// newsz, which need not be page aligned.  Returns new size or 0 on error.
uint64
uvmalloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz, unsigned long int xperm)
{
  char *mem;
  uint64 a;
  ...
  for(a = oldsz; a < newsz; a += PGSIZE){
    ...
    memset(mem, 0, PGSIZE);
    //为新的页创建物理映射
    if(mappages(pagetable, a, PGSIZE, (uint64)mem, PTE_P | PTE_W | PTE_U | PTE_MAT_CC | xperm) != 0){
      kfree(mem);
      uvmdealloc(pagetable, a, oldsz);
      return 0;
    }
...
freewalk(pagetable_t pagetable)
{
  ...
  for(int i = 0; i < 512; i++){
    ...
    if((pte & PTE_V) && (PTE_FLAGS(pte) == PTE_V)){
      // this PTE points to a lower-level page table.
      uint64 child = (PTE2PA(pte) | KERNBASE);
      freewalk((pagetable_t)child);
      pagetable[i] = 0;
    } else if(pte & PTE_V){
      #ifdef __DEBUG_FREEWALK
...
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    if((mem = kalloc()) == 0)
      goto err;
    memmove(mem, (char*)(pa | KERNBASE), PGSIZE);
    if(mappages(new, i, PGSIZE, (uint64)mem, flags) != 0){
      kfree(mem);
      goto err;
    }
...
copyin(pagetable_t pagetable, char *dst, uint64 srcva, uint64 len)
{
  ...
  while(len > 0){
    ...
    if(n > len)
      n = len;
    memmove(dst, (void *)((pa0 + (srcva - va0)) | KERNBASE), n);
...
copyinstr(pagetable_t pagetable, char *dst, uint64 srcva, uint64 max)
{
  ...
  while(got_null == 0 && max > 0){
    ...
    char *p = (char *) ((pa0 + (srcva - va0)) | KERNBASE);
    while(n > 0){
      if(*p == '\0'){
        *dst = '\0';
        got_null = 1;
