...
#define SATA_SIG_ATAPI \
    0xEB140101  // SATAPI设备，识别支持 ATAPI 协议的 SATA 设备
...
struct fis_reg_host_to_device {
    ...
    /*一个8位的位域如下指定出的整数*/
    uint8_t pmport : 4;  // 端口多路复用器编号，表示该 FIS
                         // 目标设备的端口号。在这个结构中，占用了 4
                         // 个位，表示端口号范围为 0 到 15
    uint8_t reserved0 : 3;
...
struct FisRegH2D
{
	...
	uint8_t rsv1;						// [x] reserve 
	uint8_t control;
	// Dword 4 
	uint8_t rsv2[ 4 ];					// [x] reserve 
...
struct FisPioD2H
{
	...
	uint8_t fis_type;
	uint8_t pm_port : 4;
	uint8_t rsv0 : 1;					// [x] reserve 
	uint8_t d : 1;
	uint8_t i : 1;
	uint8_t rsv1 : 1;					// [x] reserve 
	uint8_t status;
	uint8_t error;
	...
	uint8_t sector_cnt;
	uint8_t sector_cnt_exp;
	uint8_t rsv3;						// [x] reserve 
	uint8_t e_status;
	// Dword 4 
	uint16_t transfer_cnt;
	uint8_t rsv4[ 2 ];					// [x] reserve 
}__attribute__( ( __packed__ ) );
...
struct hba_command_table {
    ...
    struct hba_prdt_entry prdt_entries
        [(4096UL - 0x80U) /
         sizeof(struct hba_prdt_entry)];  // 物理区域描述符表（PRDT）条目的数组
} __attribute__((packed));
