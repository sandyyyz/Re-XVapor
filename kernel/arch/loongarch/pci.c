...
{
	printf( "pci_device_bar_dump: type: %s\n", bar->type == PCI_BAR_TYPE_IO ? "io base address" : "mem base address");
	printf( "pci_device_bar_dump: base address: %x\n", bar->base_addr);
	printf( "pci_device_bar_dump: len: %x\n", bar->length);
}
...
void pci_device_dump(pci_device_t *device)
{
	...
	printf( "pci_device_dump: vendor id:      0x%x\n", device->vendor_id);
	printf( "pci_device_dump: device id:      0x%x\n", device->device_id);
	printf( "pci_device_dump: class code:     0x%x\n", device->class_code);
	printf( "pci_device_dump: revision id:    0x%x\n", device->revision_id);
	printf( "pci_device_dump: multi function: %d\n", device->multi_function);
	printf( "pci_device_dump: card bus CIS pointer: %x\n", device->card_bus_pointer);
	printf( "pci_device_dump: subsystem vendor id: %x\n", device->subsystem_vendor_id);
	printf( "pci_device_dump: subsystem device id: %x\n", device->subsystem_device_id);
	printf( "pci_device_dump: expansion ROM base address: %x\n", device->expansion_rom_base_addr);
	printf( "pci_device_dump: capability list pointer:  %x\n", device->capability_list);
	printf( "pci_device_dump: irq line: %d\n", device->irq_line);
	printf( "pci_device_dump: irq pin:  %d\n", device->irq_pin);
	printf( "pci_device_dump: min Gnt: %d\n", device->min_gnt);
	printf( "pci_device_dump: max Lat:  %d\n", device->max_lat);
	int i;
	for (i = 0; i < PCI_MAX_BAR; i++) {
		/*if not a invalid bar*/
		if (device->bar[i].type != PCI_BAR_TYPE_INVALID) {
			printf( "pci_device_dump: bar %d:\n", i);
...
static void pci_scan_device(unsigned char bus, unsigned char device, unsigned char function)
{
	...
	printf( "pci_scan_device: pci device at bus: %d, device: %d function: %d\n", bus, device, function);
...
void pci_enable_bus_mastering(pci_device_t *device)
{
	...
	// #if DEBUG_LOCAL == 1
	printf( "pci_enable_bus_mastering: before command: %x\n", val);    
	//#endif
	val |= 4;
	...
	pci_read_config(PCI_BASS_ADDRESS0,device->bus, device->dev, device->function, PCI_STATUS_COMMAND, &val);
	//#if DEBUG_LOCAL == 1
	printf( "pci_enable_bus_mastering: after command: %x\n", val);    
	//#endif
}
