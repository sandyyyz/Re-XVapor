...
/*打包请求*/
 ...
 void block_make_request(int cmd, uint64_t base_addr, uint64_t count, uint64_t buffer,uint8_t port_num)
{
...
{
    /*struct block_device_request_packet * pack= */
    block_make_request(ATA_CMD_WRITE_DMA_EXT,base_addr, count, buffer,port_num);
    //ahci_submit(pack);
}
