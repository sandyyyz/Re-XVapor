...
- ✅ 多核调度与 HART 管理
- ✅ 进程家族树管理（支持 `getppid`、`waitpid`、`killpg` 等）
- ✅ 支持内核级线程与 `clone` 系统调用
- ✅ 支持 Futex，采用哈希表加速队列唤醒机制
- ✅ 条件变量、信号量同步原语支持
- ✅ 完善的信号机制：支持 signal mask、sigaction、信号队列等
- ✅ 虚拟文件系统（VFS）架构 + ext4 文件系统挂载与访问
- ✅ 支持用户态 ELF 动态链接，兼容 glibc/musl 程序
- ✅ 支持管道、共享内存、socketpair 等进程间通信方式
- ✅ 提供自动化系统调用表生成工具
- ✅ 支持 QEMU 虚拟化运行，支持 RISC-V 和 LoongArch 架构
